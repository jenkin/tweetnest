<?php
	...
	function tweetHTML($tweet, $tabs = 4){
		...
		$d  =   $t . "<div id=\"tweet-" . s($tweet['tweetid']) . "\" class=\"tweet" . (($tweet['type'] == 1) ? " reply" : "") . (($tweet['type'] == 2) ? " retweet" : "") . "\">\n" . 
				...
				$t . "\t<p class=\"meta\">\n" . $t . "\t\t<a href=\"//twitter.com/" . s($rt ? $retweet['screenname'] : ($tweet['screenname'] ?: $tweet['userid'])) . "/statuses/" . s($rt ? $retweet['tweetid'] : $tweet['tweetid']) . "\" class=\"permalink\">" . date("g:i A, M jS, Y", ($rt ? $retweet['time'] : $tweet['time'])) . "</a>\n" . 
