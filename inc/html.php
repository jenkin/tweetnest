<?php
	...
	function tweetHTML($tweet, $tabs = 4){
		...
		$d  =   $t . "<div id=\"tweet-" . s($tweet['tweetid']) . "\" class=\"tweet" . (($tweet['type'] == 1) ? " reply" : "") . (($tweet['type'] == 2) ? " retweet" : "") . "\">\n" . 
                ...
                "at <span class=\"via\"><a target=\"_blank\" href=\"http://twitter.com/" . s($rt ? $retweet['screenname'] : ($tweet['screenname'] ?: $tweet['userid'])) . "/statuses/" . s($rt ? $retweet['tweetid'] : $tweet['tweetid']) . "\" class=\"permalink\">" . date("g:i A, M jS, Y", ($rt ? $retweet['time'] : $tweet['time'])) . "</a></span>\n" . 
				...
				((!$rt && $inReplyToTweetId) ? $t . "\t\t<a target=\"_blank\" class=\"replyto\" href=\"http://twitter.com/" . s($tweetextra['in_reply_to_screen_name']) . "/statuses/" . s($inReplyToTweetId) . "\">in reply to " . s($tweetextra['in_reply_to_screen_name']) . "</a>\n" : "") . 
